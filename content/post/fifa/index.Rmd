...
image:
  caption: "[FIFA](https://external-preview.redd.it/-mq6B0cL4Y9ZnNhXZ2pMSr8JUIwyWnjI8P4M0iv0nE4.jpg?auto=webp&s=f4ad537dfeb55b1857bb6afdd6eb2bde01cd1a2a)"
  focal_point: ""
  preview_only: false
tags: 
- fifa
summary: "These data were reconstructed by Gemma Edwards using FIFA's Committee 2006 activity report. Each actor was tied to a standing committee in a bimodal network. The data procurred from UCINET's site (https://sites.google.com/site/ucinetsoftware/datasets/covert-networks/fifa) stored these networks as one-mode graphs in which actors (people) are inteconnected via 'co-membership' ties."
---
...
```{r}
proto_net <- COREnets::get_data("fifa")
...
for (i in seq_along(vis_nets)) {
 htmlwidgets::saveWidget(
   vis_nets[[i]],
   file = sprintf("%s/FIFA%s.html", temp_dir, i)
  )
}

vis_paths <- dir(temp_dir, pattern = "FIFA\\d+\\.html$",
                 full.names = TRUE)
